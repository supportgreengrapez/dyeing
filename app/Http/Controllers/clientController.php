...
class clientController extends Controller
{
    ...
    public function dyeing_list()
    {
        ...
        return view('client.dyeing_list', compact('result','result2'));
    }
    ...
    public function dyeing_form(Request $request)
    {
        ...
        DB::insert("insert into dyeing (user_id,tahn,date,weight,quality,quantity,party_lot_no,send_to,color,bl,challan_no,folding,dyeing_lot,remarks,unit,material,status)  values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", array(
            $user_id,
            $tahn,
            $date,
            $weight,
    ...
    public function search_dyeing(Request $request)
    {
        ...
        if (!empty($r_name) && !empty($s_name))
        {
            $result = DB::select("select * from dyeing where date BETWEEN '$s_date' AND '$e_date' and recieved_from = '$r_name' and send_to = '$s_name'");

            return view('client.dyeing_list', compact('result', 'result1'));
        }
        elseif (!empty($s_name))
        {
